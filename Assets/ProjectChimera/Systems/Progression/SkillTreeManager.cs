...
namespace ProjectChimera.Systems.Progression
{
    ...
    public class SkillTreeManager : ChimeraManager
    {
        ...
        /// <summary>
        /// Get the overall skill level of the player across all skill categories
        /// </summary>
        /// <returns>Overall skill level as a float</returns>
        public float GetOverallSkillLevel()
        {
            if (_skillStates == null || _skillStates.Count == 0) return 0f;
            
            float totalLevel = 0f;
            int skillCount = 0;
            
            foreach (var skillState in _skillStates.Values)
            {
                totalLevel += skillState.CurrentLevel;
                skillCount++;
            }
            
            return skillCount > 0 ? totalLevel / skillCount : 0f;
        }
    }
